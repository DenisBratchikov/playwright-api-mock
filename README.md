...
const apiMock = new ApiMockPlugin(page, {
  ...
  resolveVariant: (req) => (req.url().includes('users/me') ? process.env.USER_TIER : undefined),
  mode: 'auto',
});
```
...
- Legacy flat files are read automatically; use the CLI to migrate permanently.
...
```
pwamock migrate path/to/snapshots.json   # legacy -> v2 format
...
## 🔄 Migrating from v0.x
- Existing flat snapshot files are read automatically.
- Use `pwamock migrate path/to/api_snapshots.json` to persist them in the new metadata-rich format.
- Default key now includes the HTTP method + normalized URL (with deterministic query ordering); you can keep legacy semantics via a custom `keyFn` if needed.
...
- Maintenance utilities: `migrateSnapshots`, `validateSnapshots`, `snapshotStats`
