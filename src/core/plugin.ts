 ...
 * ⚠️ PERFORMANCE: Full JSON stringify/parse is expensive for large bodies
 * Consider: structured clone, shallow clone, or only clone when needed
 */
...
export class ApiMockPlugin {
  ...
  private mergeConfig(
    ...
    const mode = extra.mode ?? base.mode ?? "auto";
   ...
   * ⚠️ PERFORMANCE: Evaluates ALL rules even if first matches
   * Consider: sequential evaluation with early return
   */
  private findRule(config: PluginConfig, req: Request) {
    return Promise.all(
      (config.rules ?? []).map(async (rule) => ({
        rule,
        matches: await resolveMatch(rule.match, req),
      }))
  ...
  private onMissingBehavior(config: PluginConfig) {
    return config.onMissingSnapshot ?? DEFAULT_ON_MISSING[config.mode];
  }
  ...
  async record(configOverwrite?: Partial<PluginConfig>): Promise<void> {
    const effectiveConfig = configOverwrite
      ? this.mergeConfig(this.config, configOverwrite)
      : this.config;
    const store = configOverwrite
      ? new SnapshotsStore(effectiveConfig)
      : this.store;
    ...
    await this.page.route(effectiveConfig.urlMatch, async (route) => {
      ...
      const normalizedUrl = await this.normalizeUrl(
        request.url(),
        effectiveConfig,
        rule,
        context
      );
      const variantValue = await this.resolveVariant(
        effectiveConfig,
        rule,
        context
      );
      const variant = variantToString(variantValue);
      const shouldHashBody = this.shouldIncludeBodyHash(
        request.method(),
        effectiveConfig
      );
      const requestBody = shouldHashBody
        ? await this.extractBody(request, effectiveConfig)
        : undefined;
      const bodyForHash =
      ...
      const bodyHash =
        shouldHashBody && bodyForHash !== undefined
          ? effectiveConfig.keyStrategy?.bodyHashFn?.(bodyForHash) ??
            hashObject(bodyForHash)
          : undefined;
      ...
      const storedSnapshot = await store.getStoredSnapshot(key, request.url());
      if (storedSnapshot) {
        const baseBody = storedSnapshot.response.body;
        const transformedBody = rule?.onServeResponse
          ? await rule.onServeResponse(cloneBody(baseBody), context)
          : baseBody;

        const status = storedSnapshot.response.status;
        const headers = storedSnapshot.response.headers;
      ...
      // ⚠️ PERFORMANCE: O(n) search through all entries on every request
      // With 1000 snapshots, this is 1000 iterations per request
      // Consider: index by normalizedUrl for O(1) lookup
      const nearMatch = store
        .listEntries()
        .find((entry) => entry.request.normalizedUrl === normalizedUrl);
      if (nearMatch && effectiveConfig.onMismatch === "fail") {
        throw new Error(
          `Snapshot mismatch for ${key}; found different key ${nearMatch.key}`
        );
      }
      if (nearMatch && effectiveConfig.onMismatch === "warn") {
        this.log(
          `Snapshot mismatch for ${key}; closest match ${nearMatch.key}`
        );
      }
      ...
      try {
        ...
        const filteredHeaders =
          effectiveConfig.normalizeHeaders?.(rawHeaders) ??
          effectiveConfig.getStoredHeaders?.(rawHeaders);
